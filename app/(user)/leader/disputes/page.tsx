...
export default async function LeaderDisputesPage() {
  const { leaderGroupId, leaderGroup } = await requireLeaderWorkspace();

  const disputes = await db.checkIn.findMany({
    where: {
      ...
      OR: [
        ...
        {
          status: "FLAGGED",
          verifications: { some: { verdict: "APPROVE" } },
        },
      ],
    },
      ...
      <Card className="overflow-hidden border-l-4 border-l-yellow-500">
        <CardContent className="space-y-3 p-6">
          ...
          <h1 className="text-2xl font-black tracking-tight text-white">
            {leaderGroup.name} — Disputes ({disputes.length})
          </h1>
          <p className="text-sm text-white/50">
            Proofs with mixed peer votes that need your final decision as group leader.
          </p>
        </CardContent>
      </Card>
      ...
      {disputes.length === 0 ? (
        <Card>
          <CardContent className="p-8 text-center">
            <AlertTriangle className="mx-auto mb-3 h-10 w-10 text-yellow-400/30" />
            <div className="text-lg font-bold text-white/60">No disputes</div>
            <div className="text-sm text-white/40">All submissions in your group are settled.</div>
          </CardContent>
        </Card>
      ) : (
              ...
              <Card key={checkIn.id} className="border-l-4 border-l-yellow-500">
                <CardContent className="space-y-4 p-5">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {[...checkIn.startFiles, ...checkIn.endFiles].map((file, idx) => (
                        <a
                          ...
                          className="flex items-center gap-1.5 rounded-[4px] border border-border bg-secondary/20 px-2.5 py-1.5 text-xs text-white/60 hover:border-yellow-500/30 hover:text-yellow-400"
                        >
                  ...
                  <div className="space-y-1.5 border-t border-border pt-3">
                    <div className="text-[10px] font-bold uppercase tracking-wider text-white/30">Vote Timeline</div>
                    {checkIn.verifications.map((v, idx) => (
                      <div key={idx} className="flex items-start gap-2 text-xs">
                        ...
                        <span className="text-white/50">
                          ...
                          {v.note && ` — "${v.note}"`}
                          <span className="ml-2 text-white/30">{v.createdAt.toLocaleDateString()}</span>
                        </span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 border-t border-border pt-3">
                    <form action={resolveFlaggedSubmission}>
                      ...
                      <input type="hidden" name="returnTo" value="/leader/disputes" />
